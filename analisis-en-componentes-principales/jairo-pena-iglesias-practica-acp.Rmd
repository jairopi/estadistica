...
```{r}
## completar aquí
datos3d <- read.table(file = "data/datos3d.txt", 
                      header = FALSE, 
                      sep = ";", 
                      dec = ",", 
...
names(datos3d) <- c("X","Y","Z")

```


Representamos la nube de puntos tri-dimensionales con la instrucción plot3d de la libreria rgl. En la línea de comandos, o en nuestro editor R, escribimos
...
Finalmente, también podemos extraer del objeto datos3d.acp las desviaciones típicas de los componentes principales (es decir de las coordenadas de los puntos en el nuevo sistema de coordenadas).
...
raiz cuadrada de autovalores = desviaciones.
...
Para encontrar los valores de la primera columna realiza el cuadrado de PC1.
...
epf <- read.table(file = "data/epf13-persona.txt", 
                  header = TRUE, 
                  sep = ";", 
                  dec = ".", 
                  stringsAsFactors = FALSE, 
...
## Nada que completar
ranking <- epf %>% 
  mutate_each(funs(18 - rank(.)), contains("Grupo"), Total) #si ponemos 18 las ordena de 1 a   #17, y no de 3 a 19 como cuando ponemos 20
...
En el País Vasco, Madrid y Navarra.

Vuestros comentarios sobre el ranking de Murcia:

Se situa en la parte baja en los grupos: "Vivienda, agua, electricidad, gas y otros combustibles" y "Comunicaciones".
Se situa en la parte media en los grupos:"Artículos de vestir y calzado" y "Transporte".
...
lepf <- epf %>%
  mutate_each(funs(log(.)), contains("Grupo"))
...
```{r}
## 
v1 <- lepf.acp$rotation[ , 1]
v2 <- lepf.acp$rotation[ , 2]
v3 <- lepf.acp$rotation[ , 3]
```
...
PC1 <- x[ , 1]
PC2 <- x[ , 2]
clasificaciones <- order(PC1, decreasing = TRUE)
lepf[clasificaciones, 2]
...
ggplot(data = epf, aes(x = PC1, y = PC2))+
  geom_point(color = "blue")+
  geom_text(data = epf, aes(label = Comunidad), color = "blue", vjust = +1, size = 4)
##vjust = +1 para que el punto salga encima y centrado con respecto a al etiqueta (nombre de la comunidad autonoma)
...
```{r}
medidas <- read.table(file = "data/medidas.txt", 
                      header = TRUE, 
                      sep = ";", 
                      dec = ".", 
                      stringsAsFactors = FALSE, 
...
PC1: Promedio pecho, cintura y caderas. 
PC2: Diferencia cadera y cintura.
PC3: Diferencia cadera y cintura promediado con el pecho.
  
Con PC1 y PC2 tendremos ya toda la variabilidad estudiada, mientras que PC3 no nos servirá.
