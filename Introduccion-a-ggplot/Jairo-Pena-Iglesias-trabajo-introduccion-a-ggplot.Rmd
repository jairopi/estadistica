...
```{r, message = FALSE}
## cargamos los paquetes "lubridate", "dplyr" y "ggplot2"
library("lubridate")
library("dplyr")
library("ggplot2")
...
```{r}
### insertar el código para importar (cargamos los datos):
...
ggplot(data = vivienda, aes(x = Global_active_power)) + geom_histogram()
...
```{r}
ggplot(data = vivienda, aes(x = factor(anyo), y = Voltage)) + geom_boxplot()
...
```{r}
p <- ggplot(data = vivienda, aes(x = DateHour, y = Global_active_power)) + geom_line()
p

```

Se observa algo destacable?
La gráfica queda muy cercana a 0 en una pequeña franja entre los años 2008 y 2009, por lo que hemos de suponer que no hay consumo o ha habido algun problema en la medicion del mismo.
...
p + geom_line(colour = "blue") + 
  labs(title = 'Evolución temporal de la potencia global activa consumida, 2006-2012',x = 'Fecha', y ='Potencia global activa (kW)')

#Guardamos la gráfica
ggsave(filename = "figures/potencia-global-activa-2006-2012.png", plot= last_plot(), width = 10, height = 7)
...
```{r}
viviendaJD2008 <- vivienda %>%
filter(anyo==2008, between(mes, 07, 12))

x <- ggplot (data = viviendaJD2008, aes (y = Global_active_power, x = DateHour)) + geom_line(colour = "blue") + labs(title = 'Evolución temporal de la potencia global activa consumida, 2006-2012' , x = 'Fecha' , y ='Potencia global activa (kW)') 

x

#Guardamos la gráfica
ggsave(filename = "figures/potencia-global-activa-julio-diciembre-2008.png", plot= last_plot(), width = 10, height = 7)


```

Veís algo destacable? ¿Teneís alguna explicación?
Parece ser que el consumo de potencia cae a practicamente 0 en periodo vacacional. Puede ser que los inquilinos se fueran de vacaciones.
...
```{r}
viviendaVerano0710 <- vivienda %>%
 filter(between(mes, 07, 09), between(anyo, 2007, 2010))

y <- ggplot (data = viviendaVerano0710, aes (y = Global_active_power, x = DateHour)) + geom_line() + 
  labs(title = 'Evolución temporal de la potencia global activa consumida en Verano, 2006-2012.' , x = 'Fecha' , y ='Potencia global activa (kW)') + 
  facet_grid(anyo ~ .)
y
...
No, quería el eje x con los meses y el eje y con los años.

> Para mejorar esta gráfica, introducimos una columna que sea el resultado de restar a DateHour
...
```{r}
viviendaVerano <- vivienda %>%
 filter(between(mes, 07, 09), between(anyo, 2007, 2010)) %>%
 mutate(DateHourBase = DateHour - years(anyo))
...
```{r}
q <- ggplot (data = viviendaVerano, aes (y = Global_active_power, x = DateHourBase)) +
  geom_line() + 
  labs(title = 'Evolución temporal de la potencia global activa consumida, 2006-2012.' , x = 'Fecha' , y ='Potencia global activa (kW)') + facet_grid(anyo ~ .)
q
...
resumen.horario <- vivienda %>%
group_by(hora) %>%
summarise(Global_active_power = mean(Global_active_power, na.rm = TRUE),n = n())
...
z <- ggplot(data = resumen.horario, aes(x = hora, y = Global_active_power)) + 
...
## de la potencia
p + geom_text(aes(label = formatC(Global_active_power,3), vjust = 0))
```
...
```{r}
resumen.horario.dia %>%
ggplot(aes(x = hora, y = Global_active_power,fill = diasemana)) +
...
Mientras están dormidos los inquilinos de la casa hay poco consumo, y se dispara cuando se despiertan. El consumo vuelve a bajar durante el horario de trabajo y al llegar la noche vuelve a haber una subida. Suponemos que los inquilinos no trabajan el sabado ni el domingo, ya que sus consumos son distintos.
